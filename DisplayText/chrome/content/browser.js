	...
	if (!author || author === 'none' || author == "RSS error")
	{
		if (!(author = FindAuthor()))
		{
			if (publication && publication != 'none'){
				...
				AuthorNotFound(doc.getElementById('publication_id'), false, 'Enter source name for information');
			}

			AuthorNotFound(doc.getElementById('info_id'), true, 'Enter author name for info');
			return;
		}
		else 
		{
...
function wrongAuthor ( )
{
	...
	AuthorNotFound(doc.getElementById('info_id'), true, "Enter name of author");
}
...
function wrongSource ( )
{
	...
	// add lookup elements to box
	AuthorNotFound(doc.getElementById('publication_id'), false, "Enter name of source");
}
